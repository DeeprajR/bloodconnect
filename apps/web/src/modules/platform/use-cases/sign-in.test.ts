...
import { argon2Hasher, nodeTokens } from '../adapters/crypto';
import { anonymousActor } from '../domain/authorization';
import type { UseCaseContext } from '../context';
import { signIn } from './sign-in';
import { resolveActor, signOut } from './sessions';
...
describe.skipIf(!testUrl)('signIn (§8.1, §13, §15)', () => {
  ...
  it('answers identically for a wrong password, an unknown address, a deactivated account and one never activated (§15)', async () => {
    const attempts = await Promise.all([
      signIn(context(), { email: 'active@blood-connect.invalid', password: 'wrong' }),
  ...
  it('survives a restart, because the counter is a row and not memory (§3)', async () => {
    await signIn(context(), { email: 'active@blood-connect.invalid', password: 'wrong' });
  ...
  it('clears the account counter on success but never the IP counter', async () => {
    await signIn(context(), { email: 'active@blood-connect.invalid', password: 'wrong' });
    await signIn(context(), { email: 'active@blood-connect.invalid', password: PASSWORD });
