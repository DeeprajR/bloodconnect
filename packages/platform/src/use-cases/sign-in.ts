...
import { ROLES_BY_AUDIENCE } from '../domain/authorization.js';
import {
  attemptsInWindow,
  clearAttempts,
  recordFailedAttempt,
...
export type SignInInput = {
  ...
  /**
   * Which application is signing them in. The session is bound to it, and the
   * roles allowed differ: only an administrator signs in to the admin app, and
   * an administrator signs in to nothing else (§1, §9).
   */
...
export async function signIn(
  ...
  return ctx.db.transaction(async (tx) => {
    ...
    // neither form reveals which accounts exist on the other side.
    const wrongAudience =
      account !== undefined && !ROLES_BY_AUDIENCE[input.audience].includes(account.role);

    if (!account?.passwordHash || account.status !== 'active' || wrongAudience) {
      ...
      await audit({
        ...
        metadata: {
          reason: !account
            ? 'no_account'
            : wrongAudience
              ? `wrong_audience_${account.role}`
              : `status_${account.status}`,
        },
      });
    ...
    await audit({
      ...
      subjectType: 'user',
      subjectId: account.id,
      metadata: { sessionId, role: account.role, audience: input.audience },
    });
